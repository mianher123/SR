...
void up_setup_col(int *row0, int *row1, int ww, int hh, int index, int *aft){
	for(int i=0; i<hh; i++){
		if(i%3==0) row0[i]=aft[(i*2/3)*ww+index];
		else row0[i]=0;

		if(i%3==2) row1[i]=aft[((i-2)*2/3+1)*ww+index];
		else row1[i]=0;
	}
}

void up_setup_row(int *row0, int *row1, int *arr, int w, int ww, int j){
	for(int i=0; i<ww; i++){ // setup row0 and row1
		if(i%3==0) row0[i]=arr[j*w+i*2/3];
		else row0[i]=0;
		
		if(i%3==2) row1[i]=arr[j*w+(i-2)*2/3+1];
		else row1[i]=0;
	}
	
}

int up_convolusion_col(double u0[5], double u1[5], int *row0, int *row1, int ww, int hh, int index, int *aft){
	int e_aft=0;
	int temp[2];
	// i==0
...
int up_convolusion_row(double u0[5], double u1[5], int *row0, int *row1, int ww, int j, int *aft){
	int e_aft=0;
	int temp[2];
	// i==0
	temp[0]=(int)(u0[2]*row0[0] + u0[3]*row0[1] + u0[4]*row0[2]);
	temp[1]=(int)(u1[2]*row1[0] + u1[3]*row1[1] + u1[4]*row1[2]);
	aft[j*ww]=temp[0]+temp[1];
	e_aft+=aft[j*ww];
	// i==1
	temp[0]=(int)(u0[1]*row0[0] + u0[2]*row0[1] + u0[3]*row0[2] + u0[4]*row0[3]);
	temp[1]=(int)(u1[1]*row1[0] + u1[2]*row1[1] + u1[3]*row1[2] + u1[4]*row1[3]);
	aft[j*ww+1]=temp[0]+temp[1];
	e_aft+=aft[j*ww+1];
	// i==ww-2
...
void up(int *ori_R, int *ori_G, int *ori_B, int *aft_R, int *aft_G, int *aft_B, int w, int h, int ww, int hh){
	...
	for(int i=0; i<w; i++){ // do how many time in column
		...
		for(int j=0; j<hh; j++){
			aft_R[j*w+i]=(int)((double)(aft_R[j*w+i])/R_rate);
			aft_G[j*w+i]=(int)((double)(aft_G[j*w+i])/G_rate);
			aft_B[j*w+i]=(int)((double)(aft_B[j*w+i])/B_rate);
		}
	} // column finish
	...
	for(int j=0; j<hh; j++){ // do how many time in row
		...
		B_rate=(double)e_aft/(double)(B_ori*3.0/2.0);
		for(int i=0; i<ww; i++){
			aft_R[j*ww+i]=(int)((double)(aft_R[j*ww+i])/R_rate);
			aft_G[j*ww+i]=(int)((double)(aft_G[j*ww+i])/G_rate);
			aft_B[j*ww+i]=(int)((double)(aft_B[j*ww+i])/B_rate);
		}
	} // row finish
	*/
