...
void setup_col(int *row0, int *row1, int *arr, float *d0, float *d1, int j, int w, int h){
	...
	row0[h-1]=(int)(d0[0]*arr[(h-2)*w +j]+d0[1]*arr[(h-1)*w +j]);
	row1[h-1]=(int)(d1[0]*arr[(h-2)*w +j]+d1[1]*arr[(h-1)*w +j]);
	for(int i=1; i<h-1; ++i){
		/*
		if(i==0){
			row0[0]=(int)(d0[1]*arr[j]+d0[2]*arr[w +j]);
...
void setup_row(int *row0, int *row1, int *arr, float *d0, float *d1, int j, int w, int h){
	...
	row0[w-1]=(int)(d0[0]*arr[j*w +w-2]+d0[1]*arr[j*w +w-1]);
	row1[w-1]=(int)(d1[0]*arr[j*w +w-2]+d1[1]*arr[j*w +w-1]);
	for(int i=1; i<w-1; ++i){
		/*
		if(i==0){
			row0[0]=(int)(d0[1]*arr[j*w +0]+d0[2]*arr[j*w +1]);
...
void down(
	...
	for(int i=0; i<w; ++i){
		...
		// red
		setup_col(row0, row1, ori_R, d0, d1, i, w, h);
		e_aft=0;
		for(int j=0; j<hh; ++j){
			if(j%2==0) ans_R[j*w +i]=row0[3*j/2];
			else ans_R[j*w +i]=row1[3*(j-1)/2 +2];
			e_aft+=ans_R[j*w +i];
		}
		R_rate=(float)e_aft/((float)R_ori*2.0/3.0);
		for(int j=0; j<hh; ++j){
			ans_R[j*w +i]=(int)((float)ans_R[j*w +i]/R_rate);
		}
		// green
		setup_col(row0, row1, ori_G, d0, d1, i, w, h);
		e_aft=0;
		for(int j=0; j<hh; ++j){
			if(j%2==0) ans_G[j*w +i]=row0[3*j/2];
			else ans_G[j*w +i]=row1[3*(j-1)/2 +2];
			e_aft+=ans_G[j*w +i];
		}
		G_rate=(float)e_aft/((float)G_ori*2.0/3.0);
		for(int j=0; j<hh; ++j){
			ans_G[j*w +i]=(int)((float)ans_G[j*w +i]/G_rate);
		}
		// blue
		setup_col(row0, row1, ori_B, d0, d1, i, w, h);
		e_aft=0;
		for(int j=0; j<hh; ++j){
			if(j%2==0) ans_B[j*w +i]=row0[3*j/2];
			else ans_B[j*w +i]=row1[3*(j-1)/2 +2];
			e_aft+=ans_B[j*w +i];
		}
		B_rate=(float)e_aft/((float)B_ori*2.0/3.0);
		for(int j=0; j<hh; ++j){
			ans_B[j*w +i]=(int)((float)ans_B[j*w +i]/B_rate);
		}
	}
	...
	for(int j=0; j<hh; ++j){
		...
		// red
		setup_row(row0, row1, ans_R, d0, d1, j, w, hh);
		e_aft=0;
		for(int i=0; i<ww; ++i){
		...
		// green
		setup_row(row0, row1, ans_G, d0, d1, j, w, hh);
		e_aft=0;
		for(int i=0; i<ww; ++i){
		...
		// blue
		setup_row(row0, row1, ans_B, d0, d1, j, w, hh);
		e_aft=0;
		for(int i=0; i<ww; ++i){
