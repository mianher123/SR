...
using namespace cv;
int main(void){
	char *ImageName = "C:\\Users\\dada\\Desktop\\GPU_Image\\Koala_420.jpg";
	...
	int w=ori_w-(ori_w%12);
	int h=ori_h-(ori_h%12);
	...
	*******************************************************/
	...
	for(int j=0; j<hh; ++j){ // copy the ans to pImg2
		for(int i=0; i<ww; ++i){
			/*
			pImg2->imageData[(j*ww +i)*bpp +2]=ori_R[j*ww +i];
			pImg2->imageData[(j*ww +i)*bpp +1]=ori_G[j*ww +i];
			pImg2->imageData[(j*ww +i)*bpp +0]=ori_B[j*ww +i];
			*/
			/*
