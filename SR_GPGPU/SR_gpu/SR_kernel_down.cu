...
__device__ void setup_col(int *row0, int *row1, int ww, int h, int index, int *ans){
	row0[0]=(int)(tex1Dfetch(Td0, 1)*ans[index]
					+tex1Dfetch(Td0, 2)*ans[ww +index]);
	row1[0]=(int)(tex1Dfetch(Td1, 1)*ans[index]
					+tex1Dfetch(Td1, 2)*ans[ww +index]);
	row0[h-1]=(int)(tex1Dfetch(Td0, 0)*ans[(h-2)*ww +index]
					+tex1Dfetch(Td0, 1)*ans[(h-1)*ww +index]);
	row1[h-1]=(int)(tex1Dfetch(Td1, 0)*ans[(h-2)*ww +index]
					+tex1Dfetch(Td1, 1)*ans[(h-1)*ww +index]);
	for(int i=1; i<h-1; ++i){
		/*
		if(i==0){
...
__device__ void setup_row(int *row0, int *row1, int w, int index, texture<int, 1, cudaReadModeElementType> rgb){
	row0[0]=(int)(tex1Dfetch(Td0, 1)*tex1Dfetch(rgb, index*w +0)
					+tex1Dfetch(Td0, 2)*tex1Dfetch(rgb, index*w +1));
	row1[0]=(int)(tex1Dfetch(Td1, 1)*tex1Dfetch(rgb, index*w +0)
					+tex1Dfetch(Td1, 2)*tex1Dfetch(rgb, index*w +1));
	row0[w-1]=(int)(tex1Dfetch(Td0, 0)*tex1Dfetch(rgb, index*w +w-2)
					+tex1Dfetch(Td0, 1)*tex1Dfetch(rgb, index*w +w-1));
	row1[w-1]=(int)(tex1Dfetch(Td1, 0)*tex1Dfetch(rgb, index*w +w-2)
					+tex1Dfetch(Td1, 1)*tex1Dfetch(rgb, index*w +w-1));
	for(int i=1; i<w-1; ++i){
		/*
		if(i==0){
...
__global__ void run_row(int round, int *ans_R, int *ans_G, int *ans_B, int w, int h, int ww, int hh){
	...
	if(round+tid<h){
		...
		for(int i=0; i<w*2/3; ++i){
			...
			//if(ans_R[index +i]>255) ans_R[index +i]=255;
			//else if(ans_R[index +i]<0) ans_R[index +i]=0;
		}
		// green
		...
		for(int i=0; i<w*2/3; ++i){
			...
			//if(ans_G[index +i]>255) ans_G[index +i]=255;
			//else if(ans_G[index +i]<0) ans_G[index +i]=0;
		}
		// blue
